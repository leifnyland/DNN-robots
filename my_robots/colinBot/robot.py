...
def get_reward(game, bot):
    ...
    reward += bot.hp / 50  # Normalizing HP to a scale of 0 to 1

    # Damage efficiency: Reward more for causing damage without taking much
    if bot.damage_taken > 0:
        reward += (bot.damage_caused - bot.damage_taken) / 40
    else:
        reward += bot.damage_caused / 15  # Higher reward if no damage taken

    n = rg.get_neighborhood(game, bot, within=3, metric='cityblock')
    enemies_nearby = sum([x is not None for x in n['enemies']])
    teammates_nearby = sum([x is not None for x in n['allies']])

    if enemies_nearby > 1 and teammates_nearby <= 1:  # Engaging multiple enemies alone
        reward -= 0.5

    # Reward kills and penalize team deaths
    reward += bot.kills * 2
    reward -= bot.team_deaths * 2

    # Late game strategy: Encourage maintaining health and outnumbering opponents
    if last_turn(game):
        reward += (bot.hp / 50) * 2  # Additional reward for ending with high health
        team_advantage = bot.teammates - bot.opponents
        reward += team_advantage * 2  # Reward based on team balance
