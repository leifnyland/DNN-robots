...
def locationAward(location):
    return math.sqrt((location[0] - 9) ** 2 + (location[1] - 9) ** 2)/13
...
def get_reward(game, bot):
    ...
    if bot.damage_caused > 0:
        reward += bot.damage_caused * 5

    if bot.kills > 0:
        reward += bot.kills * 30

    if bot.opponent_deaths > 0:
        reward += bot.opponent_deaths * 10
    ...
    reward += (locationAward(bot.location) - 0.3) * 100
