...
def get_state(game, robot):
    ...
    # add in hp,
    neighborhood = rg.get_neighborhood(game, robot, within=1, metric='cityblock')
...
def get_reward(game, bot):
    ...
    if bot.opponent_deaths > 0:
        reward += bot.opponent_deaths * 20

    # wins
    if game.game_over and bot.teammates > bot.opponents:
        reward += 5000
    else:
        reward -= 500
    ...
    if bot.teammates > bot.opponents and reward > 0:
        reward *= 2

    # # reward for being close to friends
    # neighborhood = rg.get_neighborhood(game, bot, within=2, metric='cityblock')
    # reward += sum([x is not None for x in neighborhood['allies']]) > 0

    # reward for attacking when enemies are nearby
    # neighborhood = rg.get_neighborhood(game, bot, within=1, metric='cityblock')
    # if sum([x is not None for x in neighborhood['enemies']]) > 0 and bot.damage_caused < 1:
    #     reward -= 5
