...
def get_reward(game, bot):
    ...
    # dies
    if died(bot):
        if bot.damage_caused >= bot.damage_taken * 1.25:
            reward += 5
        else:
            reward -= 25

    # reward being in a winning position
    if bot.teammates > bot.opponents:
        reward += 20 * (bot.teammates - bot.opponents) * (game.turn ** 2) / 10000

    # reward being close to teammates
