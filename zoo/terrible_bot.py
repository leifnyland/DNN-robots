        ...
        for loc, bot in game.get('robots').items():
            if bot.player_id != self.player_id:
                if bot.hp > hp:
                    ...
                    l = loc
        # RMP
        return None if l is None else self.plan_move(game, rg.toward(self.location, l))
    ...
    def random_enemy(self, game):
        ...
        for loc, bot in game.get('robots').items():
            if bot.player_id != self.player_id:
                tmp.append(loc)
        # RMP:
        return None if len(tmp) == 0 else self.plan_move(game, rg.toward(self.location, random.choice(tmp)))
