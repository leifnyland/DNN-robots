...
class Robot:
    ...
    def act(self, game):
        ...
        for loc, bot in game.get('robots').items():
            if bot.get('player_id') != self.player_id:
                ...
                if "spawn" in rg.loc_types(
                        bot.get("location")):  # The friendly wants to get out of spawn, make way for it
    ...
    def flee(self, game):
        ...
        possible_dests = [(self.location[0] + d[0], self.location[1] + d[1]) for d in directions]
        adj_enemy_hp = 0
        adj_ally_hp = 0
...
def get_adjacent(location):
    directions = ((1, 0), (0, -1), (-1, 0), (0, 1))
    adj = ((location[0] + d[0], location[1] + d[1]) for d in directions)
    return adj
