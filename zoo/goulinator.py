...
class Robot:
    def act(self, game):
        ...
        targets = sorted(game.get('robots').values(), key=lambda v: (v.get('player_id') == self.player_id, rg.wdist(self.location, v.get('location')), v.get('hp')))
        ...
        upto = 0
        inrange = len(list(filter(lambda x: rg.wdist(x.location, self.location), targets)))
        ...
        for v in targets:
            ...
            around = list(filter(lambda x: rg.wdist(x, k) <= rg.wdist(self.location, k), around))

            # And once again filter them out to see if they are taken
            around = list(filter(lambda x: not x in positions, around))
