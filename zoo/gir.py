...
class Robot:
    ...
    def act(self, game):
        ...
        if turn == 1:
            self.setup()
            Robot.COMMANDS = defaultdict(dict)
            Robot.ATTACKS = defaultdict(lambda: defaultdict(int))
        ...
        # rmp
        if turn in self.COMMANDS and self.robot_id in self.COMMANDS[turn]:
            command = self.COMMANDS[turn][self.robot_id]
        else:
            command = ['guard']
        valid_locations = rg.locs_around(
            self.location, filter_out=['invalid', 'obstacle'])
        if len(command) > 1 and command[1] not in valid_locations:
            return ['guard']
        return command

    @classmethod
    def setup(cls):
        cls.SPAWN_LOCATIONS.update(rg.settings.spawn_coordinates)
    ...
    def compute_commands(self, robots, turn):
        my_bots = dict((b.location, b) for b in robots.values() if self.is_my_bot(b))
        enemy_bots = dict((b.location, b) for b in robots.values() if self.is_enemy_bot(b))
